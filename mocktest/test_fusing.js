...
module.exports = async function(callback) {
  ...
  await getSpotPrice(ETHUSDCAmm, web3);
  await getTwapPrice(ETHUSDCAmm, 3600, web3);
  ...
  console.log("\n  ===  user 0 many long === \n");
  for(let i=0; i<10; i++) {
    // await advanceTime(web3, 160)
    await openPosition(house, 
      ETHUSDCAmm.address, 
      0,   //  
      web3.utils.toWei("10000"),  // 1000 usdc
  ...
  await getSpotPrice(ETHUSDCAmm, web3);
  await getTwapPrice(ETHUSDCAmm, 3600, web3);
  await isInFusing(ETHUSDCAmm)

  await openPosition(house, 
    ETHUSDCAmm.address, 
    0,   // short 
    web3.utils.toWei("100"),  // 1000 usdc
    web3.utils.toWei("2"),    // 5 leverage
    web3.utils.toWei("0"),  //minBaseamount( for slippage)
    accounts[0],
    web3
  ...
  await addMargin(house, web3.utils.toWei("2"), accounts[0])

  console.log("\n  ===  delay 50 min === \n"); 
  await advanceTime(web3, 1200)
